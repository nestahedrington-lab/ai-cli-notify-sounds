...
try:
    ...
    notification_msg = p.get("message", "") or ""
    notification_title = p.get("title", "") or ""
    ...
    stop_reason = os.environ.get("CLAUDE_STOP_REASON", "")
    ...
    if event == "Notification":
        ...
        skip = not needs_action
        action_text = notification_msg or notification_title or "Claude needs your attention"
        label = f"{project} | {action_text[:80]}" if project else action_text[:80]
    elif event == "StopFailure":
        is_err = True
    elif event == "Stop":
        if stop_reason:
            is_err = stop_reason != "end_turn" or is_error_message(msg)
        else:
